...
case class WaitTablePop[T <: Data](p : WaitTableParameter, contextType : HardType[T]) extends Bundle{
  val sel = Bits(p.selCount bits)
  val selPriority = Bits(p.selCount bits)
  val context = contextType()
}
...
case class WaitTableIo[T <: Data](p : WaitTableParameter, slotContextType : HardType[T]) extends Bundle{
  ...
  val pop = Vec.fill(p.slotCount)(master Stream(WaitTablePop(p, slotContextType)))
}
...
class WaitTable[T <: Data](p : WaitTableParameter, slotContextType : HardType[T]) extends Component{
  ...
  val lines = for(line <- 0 until p.lineCount) yield new Area{
    val ways = for(way <- 0 until p.wayCount) yield new Area{
      ...
      when(pop.ready){ popSel := 0; popSelPriority := 0}

      pop.valid := popValid
      pop.sel := popSel
      pop.selPriority := popSelPriority
      pop.context := context
    }
    val free = !ways.map(_.valid).orR
    val refill = KeepAttribute(io.push.valid && free && io.push.line(line))

    io.push.ready setWhen(refill)
