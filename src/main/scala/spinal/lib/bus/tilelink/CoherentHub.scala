  ...
  val upstream = new Area{
    ...
    val c = new Area{
      ...
      val merged = new Area{
        ...
        val buffer = bufferFork.translateWith(bufferFork.data).queue(cBufferCount*wordsPerBlock).m2sPipe()
  ...
  val probe = new Area{
    ...
    val cmd = new Area{
      ...
      val buses = for ((node, nodeId) <- p.nodes.zipWithIndex.filter(_._1.withBCE)) yield new Area {
        ...
        val slotSourceChannelHits = B(p.nodes(nodeId).m.masters.map(m => m.mapping.map(_.id.hit(ssl)).orR))
...
//    )
  ...
  val ctrl = new Area{
    ...
    val stageables = List(SLOT_ID, ADDRESS, SIZE, READ, WRITE, SOURCE, FROM_C)
